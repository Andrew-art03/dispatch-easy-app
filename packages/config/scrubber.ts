/**
 * The secret scrubber — the registry rail and the pattern rail.
 *
 * WHY THIS LIVES IN `packages/config/` AND NOT IN `agents/` (1F/N-1).
 *
 * It was written at 1C as part of `agents/secrets.ts`, beside `readSecret()`,
 * because a skill reading a secret and that secret being redacted afterwards are
 * two halves of one guarantee. Then P-1C-2 put the keys `packages/config/db.ts`
 * hands out onto the same primary rail — which meant db.ts, the ONE sanctioned
 * client factory that every app process, Edge Function and script is supposed to
 * call, imported `agents/secrets.ts`.
 *
 * That import was not inert. `agents/secrets.ts` imports `agents/agent-process.ts`,
 * which runs `declareProcessKind("agent")` at module init. So importing the
 * factory declared the whole process an agent, and an app or edge process using
 * it as designed then had two outcomes, both wrong: set the `EZ_PROCESS_KIND=app`
 * opt-out the docs require and die on the contradiction, or leave it alone and be
 * refused the production client it is legitimately allowed to hold.
 * `scripts/migrate.ts` already imports db.ts, so 1D inherited it as well.
 *
 * The registry is not agent-specific. It is process-wide, it has no agent
 * semantics in it, and both sides need the SAME one — so it lives with the other
 * process-wide config, and `agents/secrets.ts` re-exports it. Every existing
 * import keeps working and the app side no longer reaches `agents/**` at all.
 * `scripts/assert-agent-imports.mjs` now checks that direction too, so the edge
 * cannot come back the next time something needs a registry.
 *
 * This module imports nothing. That is deliberate: it is the property that makes
 * it safe for db.ts to depend on.
 */
