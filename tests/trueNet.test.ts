...
function load(overrides: LoadOverrides = {}): TrueNetInput {
  ...
  return {
    ...BASE,
    ...rest,
    // `exactOptionalPropertyTypes` (main's tsconfig) refuses an explicit `undefined` where a
    // property is merely optional. Destructuring produces exactly that, and "possibly absent"
    // is the intent here, so it is stated as a cast rather than by loosening RevenueKnobs.
    revenueLines: revenueLinesFrom({ linehaul, passThrough, other, lines } as RevenueKnobs),
  };
}
...
describe("[R3-3] membership is exclusive and total, and 'reimbursables' is not a category", () => {
  ...
  it("an empty or absent line list is NEEDS_INPUT — a load nobody entered is not a zero-revenue load", () => {
    for (const lines of [[], undefined]) {
      // Passing `undefined` IS the case under test, so the cast preserves it rather than
      // hiding it; without it exactOptionalPropertyTypes rejects the very input being probed.
      const result = calculateTrueNet({ ...load(), revenueLines: lines } as Partial<TrueNetInput>);
      expect(result.status).toBe("NEEDS_INPUT");
