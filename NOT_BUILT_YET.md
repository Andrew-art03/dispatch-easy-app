...
| **C-3 — `src/lib/supabase.ts` builds its own client, so `check:db-boundary` is red on `main`** | It is a front-end file and `src/**` is out of this ticket's scope (BUILD_DEFAULTS §2). Enforcement against a *new* violation is already live: `tests/db-boundary.test.ts` pins the baseline **by name** (`KNOWN = ["src/lib/supabase.ts"]`) and runs inside `bun run test`, so a second offender is red immediately. The CLI has no baseline concept, which is why it reports the known one. Neither fixed nor hidden here | A front-end ticket routing the app through `createDb()`. Until then the rail's red line is expected and its meaning is: "still one, still that one" |
...
| **A schedule for `sweep_orphaned_agent_calls()` and `purge_idempotency()`** | Both functions are defined in migration 0003 and neither is wired to a timer. A timer needs the 6F worker, which is its own ticket | The `job` table exists as of 0003; the runner that drains it is 6F |
| **`load.exception`** | SPEC 2C §1 revokes `update (status, exception) on load`. Neither column exists in the frozen schema — the state column is `state`, and `exception` is nowhere in `schema.sql` (O-9). A frozen schema is not widened as a side effect of a hardening pass | Slice 6 (3A). If the state machine genuinely needs an exception column, it arrives as its own numbered migration with Andrew's review. `TODO(3A)` is in 0003's SECTION D |
...
| **An invitation flow — joining an org that already has users** | O-13's fix (Slice 3) allows a self-insert into `"user"` only for an org with no users yet, which is exactly the first-login bootstrap. A second person joining an existing carrier therefore has no path, and there was no invite table, token or acceptance step to preserve — the old path was the hole | A ticket for `invitation` (org_id, email, role, token, expires_at, accepted_at) plus an acceptance endpoint. Until it exists, a pilot carrier is one account |
| **Narrowing `org_bootstrap_insert`** | It is `with check (true)`, so any authenticated user can create org rows. Sign-up creates the org *before* the caller has a user row, so `auth.org_id()` is NULL at that moment and nothing narrower can be written without also deciding what sign-up looks like — a product decision. It grants no access to an existing org; O-13 closed that path | Andrew's call on sign-up. Proposed shape: `with check (not exists (select 1 from "user" u where u.id = auth.uid()))` — you may bootstrap an org only while you have no user row at all. Exempted by name, with this reasoning, in `supabase/rls.exemptions.json` |
| **The storage policy on the `docs` bucket** | SPEC 2B specifies `(storage.foldername(name))[1] = auth.org_id()::text`. `storage.objects` is not in this repo's migration chain and the bucket does not exist yet | 5A (unsigned) creates the bucket. The policy lands with it, not before |
